...
class CamGui(QWidget):
    ...
    def make_temppanel(self):
        ...
        timings.addWidget(self.SaveImages,4,1)#3
...
#            Ty = paray[0]*(ms/kb)
    ...
    async def fit_routine(self,adT):
        ...
        try:
            if self.SaveImages.isChecked():
                savelocation = self.savepath + '\dT_' + str.replace('{:3.3f}'.format(adT),'.','_') + '_ms'
                if not os.path.exists(savelocation):
                    os.makedirs(savelocation)
                for i in range(len(self.resultarray)):
                    if keeplist[i]:
                        np.savetxt(savelocation + '\_{:d}.txt'.format(i), data, delimiter=",")
        except Exception as e:
            print(e)
            print(savelocation)
