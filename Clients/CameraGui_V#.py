...
class CamGui(QWidget):
    ...
    def initialize_GUI(self):
        ...
        self.tabwid.addTab(settingspanel,'Settings')
        self.tabwid.addTab(self.Srbrainpanel,'Timings')
        self.tabwid.addTab(logpanel,'Log')
        self.tabwid.addTab(temppanel,'Temperature')


        layout = QGridLayout()
        layout.addWidget(imagepanel,0,0)

        layout.addWidget(self.tabwid,1,0)
    ...
    def log_clicked(self,state):
        if state:
            ...
            self.sender().setText('Logging in progress')
        ...
        if not state:
            self.sender().setText('Press to log')
            self.stoplog.set()
    ...
    def make_buttonspanel(self):
        panel = QFrame()
        self.savebutton = QPushButton('Start Temperature Measurement')
        ...
        layout = QGridLayout()
        layout.addWidget(self.savebutton,0,0)
    ...
    def make_inspectionpanel(self,stop):
        print('got here31')
        panel = QFrame()
        layout = QVBoxLayout()
        inspectionlayout = QHBoxLayout()
        print('got here32')
        self.inspectionboxlist = []
        try:
            for i in range(len(self.resultarray)):
                ...
                self.inspectionboxlist.append(checkbox)
                sublayout.addWidget(checkbox)
                sublayout.addWidget(pw)

                inspectionlayout.addLayout(sublayout)
        except Exception as e:
            print(e)

        print('got here33')
        inspectionbutton = QPushButton('Press to complete inspection')
        inspectionbutton.clicked.connect(lambda: stop.set())

        layout.addWidget(inspectionbutton)
        layout.addLayout(inspectionlayout)
        print('got here34')
    ...
    async def startTempMeas(self,Tlist):
        ...
        self.progressbar.setRange(0,len(Tlist)*10*2)
        ...
        for aT in Tlist:
            ...
            #self.TempDatax.addPoints([aT],[xsig])
            #self.TempDatax.addPoints([aT],[ysig])
    ...
    async def fit_routine(self):
        ...
        for i in range(len(self.resultarray)):
            ...
            try:
                ...
                #poptx = np.random.randn(4)
                #popty = np.random.randn(4)
        ...
        print('got here11')
        try:
            inspectionstop = asyncio.Event()
            inspectiontab = self.make_inspectionpanel(inspectionstop)
            self.tabwid.addTab(inspectiontab,'Inspection')
            self.tabwid.setCurrentWidget(inspectiontab)
        except Exception as e:
            print(e)
        print('got here22')
        while not inspectionstop.is_set():
            await asyncio.sleep(0.3)

        keeplist = [i.isChecked() for i in self.inspectionboxlist]
        self.tabwid.removeTab(inspectiontab)
        inspectiontab.deleteLater()

        fitvalues = fitvalues[keeplist]
        #PopupInspection(self.resultarray,self.LUT)))
        #self.Imagecheck = PopupInspection(self.resultarray,self.LUT)
        ...
        #sorted_results = fitvalues[ ~np.any( ~(abs(fitvalues)>(meanval + 4*stdval))),:] #Pure magic (sorts any rows array where some of the fitvalues are more than 4 sigma from the mean)

        meanval = np.mean(fitvalues,axis=0) #new mean values
        stdval = np.std(fitvalues,axis=0) #new std values
        
        meanim = np.mean(self.resultarray,axis=0)

        self.fitim.setImage(meanim)
        #self.fitcurve.setData(_gaussian((X,Y),*meanval).reshape(xlen,ylen))
        #self.fitcurve.setLevel(meanval[5]-stdval[5])

        #self.fitcurve.setData(meanim)
        #self.fitcurve.setLevel(10)
